...
import com.quane.little.language.{LogicalOperation, Logical, Conditional, Getter}
...
class TestExpressionService extends FlatSpec with ShouldMatchers {
  ...
  it should "find if/else expressions" in {
    ...
    val expression = service.findExpression(id)
    expression should be(new Conditional(new Logical(Value(1), LogicalOperation.Equals, Value(1))))
  }
